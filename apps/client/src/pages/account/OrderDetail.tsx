...
import { Card, CardBody, CardHeader, Spinner, Button, Chip, Divider, Table, TableHeader, TableColumn, TableBody, TableRow, TableCell } from '@heroui/react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">{t('account-order', 'Order')} #{order.number}</h1>
      ...
      <Card>
        <CardHeader className="flex gap-3 justify-between">
          <h2 className="text-lg font-semibold">{t('account-order-details', 'Order Details')}</h2>
          <Chip
        ...
        <CardBody className="gap-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-sm text-gray-500">{t('account-date', 'Date')}</p>
              <p className="font-semibold">{new Date(order.created_at).toLocaleDateString()}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">{t('account-currency', 'Currency')}</p>
              <p className="font-semibold">{order.currency}</p>
            </div>
            {order.shipped_at && (
              <div>
                <p className="text-sm text-gray-500">{t('account-shipped-at', 'Shipped')}</p>
                <p className="font-semibold">{new Date(order.shipped_at).toLocaleDateString()}</p>
              </div>
            )}
            {order.tracking_number && (
              <div>
                <p className="text-sm text-gray-500">{t('account-tracking', 'Tracking')}</p>
                <p className="font-semibold">{order.tracking_number}</p>
                {order.tracking_url && (
                  <a href={order.tracking_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 text-sm">
      ...
      <Card>
        <CardHeader>
          <h2 className="text-lg font-semibold">{t('account-items', 'Items')}</h2>
        </CardHeader>
        <Divider />
        <CardBody>
          <Table>
            ...
            <TableBody>
              {order.items.map((item, idx) => (
                <TableRow key={idx}>
                  ...
                  <TableCell>${(item.unit_price_cents / 100).toFixed(2)}</TableCell>
                  <TableCell>${(item.qty * item.unit_price_cents / 100).toFixed(2)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
