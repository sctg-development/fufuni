...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">{t('account-welcome', 'Welcome')}, {profile.name || profile.email}</h1>
      </div>
        ...
        <Card>
          <CardHeader className="flex gap-3">
            <div className="flex flex-col">
              ...
              <p className="text-gray-500">{t('account-total-orders', 'Total Orders')}</p>
            </div>
          </CardHeader>
        </Card>
        ...
        <Card>
          <CardHeader className="flex gap-3">
            <div className="flex flex-col">
              ...
              <p className="text-gray-500">{t('account-total-spent', 'Total Spent')}</p>
            </div>
          </CardHeader>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="flex gap-3 justify-between">
            <h2 className="text-lg font-semibold">{t('account-recent-orders', 'Recent Orders')}</h2>
          ...
          <CardBody>
            {profile.order_count === 0 ? (
              <p className="text-gray-500">{t('account-no-orders', 'No orders yet')}</p>
            ) : (
              <p>{t('account-last-order', 'Last order')}: {profile.last_order_at ? new Date(profile.last_order_at).toLocaleDateString() : 'N/A'}</p>
            )}
          </CardBody>
        </Card>
        ...
        <Card>
          <CardHeader className="flex gap-3 justify-between">
            <h2 className="text-lg font-semibold">{t('account-profile', 'Profile')}</h2>
          ...
          <CardBody>
            <p>
              <strong>{t('account-name', 'Name')}:</strong> {profile.name || t('account-not-set', 'Not set')}
            </p>
            <p>
              <strong>{t('account-phone', 'Phone')}:</strong> {profile.phone || t('account-not-set', 'Not set')}
            </p>
          </CardBody>
        </Card>
      </div>
